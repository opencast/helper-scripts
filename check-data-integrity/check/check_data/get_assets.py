...
and performing checks to see if the data is consistent. If it is not, a Malformed object with the encountered errors is 
returned instead.
"""
from check.check_data import create_errors
from check.check_data.check_data import check_episode_asset_of_event, check_series_asset_of_event, \
    check_asset_equality, check_series_of_event
from check.check_data.malformed import Malformed
from check.check_data.types import AssetDescription, ElementDescription, AssetTypeDescription
from shared.data.elements import has_series
from shared.data.get_assets_from_oaipmh import get_assets_from_oaipmh
from shared.data.parse_acl import parse_acl
from shared.rest_requests.asset_requests import get_asset_of_series_from_rest, get_assets_of_event_from_rest
...
def get_assets_of_event(event, opencast_url, digest_login, series_of_event, series_asset_of_series, asset_description):
    ...
    if series_asset and not isinstance(series_asset, Malformed) and series_asset_of_series \
            and not isinstance(series_asset_of_series, Malformed):
...
def get_assets_of_oaipmh(oaipmh_record, original_episode_asset, original_series_asset, series_of_event,
    ...
    # check episode asset
    errors = check_episode_asset_of_event(episode_assets, ElementDescription.OAIPMH.format(repository), asset_description, series_of_event)
    ...
    if episode_asset and not isinstance(episode_asset, Malformed):
        episode_asset = parse_for_comparison(episode_asset, ElementDescription.OAIPMH.format(repository),  #TODO
    ...
    if not isinstance(episode_asset, Malformed) and not isinstance(original_episode_asset, Malformed):
        ...
        errors = check_asset_equality(episode_asset, original_episode_asset, ElementDescription.EVENT,
                                      ElementDescription.OAIPMH.format(repository), AssetTypeDescription.EPISODE, asset_description)
        if errors:
            episode_asset = Malformed(errors=errors)
    ...
    if series_asset and not isinstance(series_asset, Malformed) and original_series_asset \
            and not isinstance(original_series_asset, Malformed):
