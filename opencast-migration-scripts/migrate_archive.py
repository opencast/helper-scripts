...
import opencast
from opencast import externalapi, scheduler, assetmanager
from opencast.adminng import create_theme
from opencast.captureadmin import update_agent_status, set_agent_capabilities, get_agents
        ...
        try:
            ...
            if is_scheduled_event:
                migrate_scheduled_episode(
                    ...
                    tmp_dir=tmp_dir)
                print(f'Skip migrating scheduled event \'{source_event["title"]}\' (ID: {source_event["identifier"]}).')
            else:
...
def main():
    ...
    #series_ids_file = 'series_ids.txt'
    series_ids_file = None
    default_timeout = 300
        ...
        with OpencastClient(source_opencast_presentation_url,
            ...
            with OpencastClient(target_opencast_admin_url,
                ...
                with OpencastClient(target_opencast_presentation_url,
                    ...
                    if series_ids_file:
                        # migrate specified series by ID
                        with open(series_ids_file, 'r') as series_ids:
                            # read one series ID per line
                            while(True):
                                series_id = series_ids.readline().strip()
                                if not series_id:
                                    break
                                try:
                                    migrate_data(
